...
#include "GifFrameHandler.h"

#define GIF_DEBUG 0

static int streamReader(GifFileType* fileType, GifByteType* out, int size) {
    GifStream* stream = (GifStream*) fileType->UserData;
...
GifFrameHandler::GifFrameHandler(GifStream* stream) :
...
GifFrameHandler::~GifFrameHandler() {
    if (mGif) {
        DGifCloseFile(mGif, NULL);
    }
...
GifFrameState* GifFrameHandler::createState() const {
    return new GifFrameState_gif(*this);
}
...
GifFrameState_gif::GifFrameState_gif(const GifFrameHandler& gifFrameGif) :
        mFrameSequence(gifFrameGif), mPreserveBuffer(NULL), mPreserveBufferFrame(-1) {
}

GifFrameState_gif::~GifFrameState_gif() {
       delete[] mPreserveBuffer;
}

void GifFrameState_gif::savePreserveBuffer(Color8888* outputPtr, int outputPixelStride, int frameNr) {
...
void GifFrameState_gif::restorePreserveBuffer(Color8888* outputPtr, int outputPixelStride) {
    const int width = mFrameSequence.getWidth();
    const int height = mFrameSequence.getHeight();
    if (!mPreserveBuffer) {
...
long GifFrameState_gif::drawFrame(int frameNr,
...
static GifFrame* createFrame(GifStream* stream) {
    return new GifFrameHandler(stream);
}
...
static GifRegistry gRegister(gEntry);
